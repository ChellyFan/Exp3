...
class GradientReconstructor():
    ...
    def reconstruct(self, input_data, input_shape=(32,), reconstruct_dp=False, K=None, dryrun=False, eval=True, tol=None):
        ...
        x = self._init_inputs(input_shape)
        dp = torch.randn((self.config['restarts'], self.num_images, input_shape[-1]), **self.setup) if reconstruct_dp else [None] * self.config['restarts']
        eps = torch.randn((self.config['restarts'], self.num_images, K), **self.setup) if K is not None else [None] * self.config['restarts']
        scores = torch.zeros(self.config['restarts'])

        try:
            for trial in range(self.config['restarts']):
                x_trial, dp_trial, eps_trial = self._run_trial(x[trial], input_data, dp_trial=dp[trial], eps_trial=eps[trial], dryrun=dryrun)
                # Finalize
                scores[trial] = self._score_trial(x_trial, input_data, dp_trial=dp_trial, eps_trial=eps_trial)
                dp[trial] = dp_trial
                eps[trial] = eps_trial
                x[trial] = x_trial
        ...
        if self.config['scoring_choice'] in ['pixelmean', 'pixelmedian']:
            ...
            dp_optimal = dp[optimal_index]
            eps_optimal = eps[optimal_index]
        print(f'Total time: {time.time()-start_time}.')
        return (x_optimal.detach(), dp_optimal.detach() if dp_optimal is not None else None, eps_optimal.detach() if eps_optimal is not None else None), stats

    def _init_inputs(self, input_shape):
        if self.config['init'] == 'randn':
            return torch.randn((self.config['restarts'], self.num_images, *input_shape), **self.setup)
        elif self.config['init'] == 'rand':
            return (torch.rand((self.config['restarts'], self.num_images, *input_shape), **self.setup) - 0.5) * 2
        elif self.config['init'] == 'zeros':
            return torch.zeros((self.config['restarts'], self.num_images, *input_shape), **self.setup)
        else:
            raise ValueError()

    def _run_trial(self, x_trial, input_data, dp_trial=None, eps_trial=None, dryrun=False):
        x_trial.requires_grad = True
        data_trial = [x_trial]
        if dp_trial is not None:
            dp_trial.requires_grad = True
            data_trial.append(dp_trial)
        if eps_trial is not None:
            eps_trial.requires_grad = True
            data_trial.append(eps_trial)
                
        if self.config['optim'] == 'adam':
            optimizer = torch.optim.Adam(data_trial, lr=self.config['lr'])
        elif self.config['optim'] == 'sgd':  # actually gd
            optimizer = torch.optim.SGD(data_trial, lr=0.01, momentum=0.9, nesterov=True)
        elif self.config['optim'] == 'LBFGS':
            optimizer = torch.optim.LBFGS(data_trial)
        else:
            raise ValueError()
        ...
        try:
            for iteration in range(max_iterations):
                closure = self._gradient_closure(optimizer, x_trial, input_data, dp_trial=dp_trial, eps_trial=eps_trial)
                ...
                with torch.no_grad():
                    ...
                    if self.config['boxed']:
                        x_trial.data = torch.max(torch.min(x_trial, torch.ones_like(x_trial)), torch.zeros_like(x_trial))
                        if dp_trial is not None:
                            dp_trial.data = torch.max(torch.min(x_trial, torch.ones_like(x_trial)), torch.zeros_like(x_trial))

                    if self.verbose and (iteration + 1 == max_iterations) or iteration % 500 == 0:
            ...
            pass
        return x_trial.detach(), dp_trial.detach() if dp_trial is not None else None, eps_trial.detach() if eps_trial is not None else None

    def _gradient_closure(self, optimizer, x_trial, input_gradient, dp_trial=None, eps_trial=None):

        def closure():
            optimizer.zero_grad()
            self.model.zero_grad()
            # loss = self.loss_fn(self.model(x_trial), label)
            recon_batch, mu, var = self.model(x_trial, dp_trial, eps_trial)
            loss = self.model.loss_function(recon_batch, x_trial, mu, var, beta=.2)
    ...
    def _score_trial(self, x_trial, input_gradient, dp_trial=None, eps_trial=None):
