...
class GradientReconstructor():
    ...
    def reconstruct(self, input_data, labels, img_shape=(3, 32, 32), K=None, dryrun=False, eval=True, tol=None):
        ...
        if K is not None:
            # eps = torch.randn((1, K), **self.setup)
            eps = torch.randn((self.config['restarts'], self.num_images, K), **self.setup)
            # eps = torch.zeros((self.config['restarts'], self.num_images, K), **self.setup)
        ...
        try:
            for trial in range(self.config['restarts']):
                if K is not None:
                    x_trial, eps_trial, labels = self._run_trial(x[trial], input_data, labels, eps_trial=eps[trial], dryrun=dryrun)
                    # Finalize
                    scores[trial] = self._score_trial(x_trial, input_data, labels, eps_trial=eps_trial)
                    eps[trial] = eps_trial
                else:
        ...
        if K is not None:
            return (x_optimal.detach(), eps_optimal.detach()), stats
    ...
    def _run_trial(self, x_trial, input_data, labels, eps_trial=None, dryrun=False):
        ...
        try:
            for iteration in range(max_iterations):
                if eps_trial is not None:
                    closure = self._gradient_closure_vb(optimizer, x_trial, input_data, labels, eps_trial)
                else:
            ...
            pass
        if eps_trial is not None:
            return x_trial.detach(), eps_trial.detach(), labels
        else:
    ...
    def _gradient_closure_vb(self, optimizer, x_trial, input_gradient, label, eps_trial):
        
        def closure():
            optimizer.zero_grad()
            self.model.zero_grad()
            loss = self.loss_fn(self.model(x_trial, eps_trial), label)
            gradient = torch.autograd.grad(loss, self.model.parameters(), create_graph=True)
            rec_loss = reconstruction_costs([gradient], input_gradient,
                                            cost_fn=self.config['cost_fn'], indices=self.config['indices'],
                                            weights=self.config['weights'])
            ...
            if self.config['total_variation'] > 0:
                rec_loss += self.config['total_variation'] * TV(x_trial)
            # eps reg
            # rec_loss += (eps_trial).pow(2).sum() * 1e-3
            rec_loss.backward()
            if self.config['signed']:
                x_trial.grad.sign_()
                # eps_trial.grad.sign_()
            return rec_loss
        return closure

    def _score_trial(self, x_trial, input_gradient, label, eps_trial=None):
